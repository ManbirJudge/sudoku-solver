...
# --- image
subprocess.run(["grim", "./data/tmp/ss.png"], check=True)
img = cv2.imread("./data/tmp/ss.png")

print('-> Screenshot taken')

H, W = img.shape[:2]

paper_img = np.zeros((PAPER_IMG_S, PAPER_IMG_S, 1), dtype=np.uint8)
...
cv2.imwrite('./data/tmp/s4.png', paper_img)
